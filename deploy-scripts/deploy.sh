...
Options:
  ...
  -scp serviceChartPath   Service chart path (required) e.g. ../${service}/Chart
  -h                      Show this help message
EOF
}

# Parse flags
while getopts "s:e:c:scp:h" opt; do
  case $opt in
    s)  service=$OPTARG ;;
    e)  env=$OPTARG ;;
    c)  component=$OPTARG ;;
    scp) serviceChartPath=$OPTARG ;;
...
if [ -z "$serviceChartPath" ]; then
  echo "Service chart path is required. Use -scp serviceChartPath" >&2
  exit 1
fi
...
secretsChartDirectory=Infrastructure/CD/k8s/Platform/secrets/infisical/
...
helm upgrade --install $componentname ./ \
